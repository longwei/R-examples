##---------------------------------------------------------------------------------##
##   Script for R / Data Mining Boot Camp          				   ##
##                                                 				   ##
##      David Weisman                              				   ##
##      Weisman@Computer.Org                       				   ##
##                                                 				   ##
##      Boston, 2012                               				   ##
##                                                 				   ##
##   This file is provided "as is" without warranty of any kind, either		   ##
##   expressed or implied, including limitation warranties of merchantability,	   ##
##   fitness for a particular purpose, and noninfringement.			   ##
##                                                 				   ##
##   In no event shall the authors or copyright holders be liable for any claim,   ##
##   damages or other liability, whether in an action of contract, tort or	   ##
##   otherwise, arising from, out of or in connection with the software or the	   ##
##   use or other dealings in the software.					   ##
##---------------------------------------------------------------------------------##


sj = read.csv('stocksJobs.csv')

#- par() sets *global* graphical parameters.  

oldPar = par(               #- save old
  bg='cornsilk',            #- background
  fg='grey35',              #- foreground
  cex.lab=3, cex.main=0.5,  #- char expansion
  lwd=5,                    #- line width
  mar=c(par('mar')+c(0,10,0,0))) # (bot,left,top,rt)

boxplot(volat ~ recession, data=sj,
        main="Boxplot of volatility",
        xlab="Recession",
        ylab="Unemployment claims")

par (oldPar) #- restore original par() settings
