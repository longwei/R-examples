...
plot(sj$sp500, sj$jobClm, #- x and y vectors
     main="S&P versus jobless claims",
     xlab="S&P 500", ylab="initial jobless claims",
     col='gray') #- deemphasize individual points

#- Compute a linear model. (More details soon)
model = lm (jobClm ~ sp500, data=sj)

abline(model, #- plot the linear model
       col='red', lwd=2) #- thicker red line
